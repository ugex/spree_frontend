module Spree
  module FrontendHelper
    ...
    def taxons_tree1(root_taxon, current_taxon, max_level = 1)
      return '' if max_level < 1 || root_taxon.leaf?
      content_tag :ul, class: 'list-group pull-left' do
        root_taxon.children.map do |taxon|
          css_class = (current_taxon && current_taxon.self_and_ancestors.include?(taxon)) ? 'list-group-item1 active' : 'list-group-item1'
          content_tag :li, class: 'list-taxon-item' do
            link_to(taxon.name, seo_url(taxon), class: css_class) + taxons_tree(taxon, current_taxon, max_level - 1)
          end
        end.join("\n").html_safe
      end
